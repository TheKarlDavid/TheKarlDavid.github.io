// FOR FIREBASE
  ...
  db.collection("contacts").get().then((snapshot) => {
    snapshot.forEach((doc) => {
      ...
      document.getElementById('contactAbout').innerHTML+=`<a href="${doc.data().contact}"${doc.data().username}><img class="contact_logo" src="${doc.data().logo}" width="75" height="75px"></a>`;
      document.getElementById('contactAbout').innerHTML+=`<a href="${doc.data().contact}"${doc.data().username}>${doc.data().username}</a>`;   
    });
  });
